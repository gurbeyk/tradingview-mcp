...
export function registerOptionsAnalysisTools(server) {
  server.tool(
    ...
    'Deterministic, read-only directional options analysis: orchestrates options_get_chain -> strategy candidate generation -> scenario repricing -> ranking into one structured packet. NO AI/LLM logic, no narrative, no invented market assumptions — base_target_price must be supplied explicitly (never inferred from spot/analyst targets/technicals). IV shocks default to 0 with an IV_SCENARIO_NOT_SPECIFIED warning unless explicitly provided. Returns a self-contained packet (with an ai_contract and field_provenance) intended for a downstream explainer to describe without recalculating anything.',
    {
